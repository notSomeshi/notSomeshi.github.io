...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function initRankings() {
    ...
    // 次要板块是按需注入的,所以绑定必须可重复调用:已绑过的用 dataset 标记跳过
    function bindPlatTips(root) {
      (root || document).querySelectorAll('.rk-plat').forEach(function (plat) {
      if (plat.dataset.tipBound) return;
      plat.dataset.tipBound = '1';
      if (canHover) {
